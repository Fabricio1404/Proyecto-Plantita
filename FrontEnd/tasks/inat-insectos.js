// iNaturalist — Insectos (Insecta) — SIN favoritos + Listas de usuario
...
function renderListsSelect(container, currentTaxonId) {
  if (!isLoggedIn()) {
    container.innerHTML = `<div class="modal-actions"><div class="box" style="border:none;padding:0;color:#f0e1c7">Iniciá sesión para usar tus <b>Listas</b>.</div></div>`;
    return;
  }
  ...
  container.innerHTML = `
    <div class="modal-actions" style="display:grid; gap:10px">
      <div class="box" style="background:rgba(255,255,255,.05); border:1px solid #6b3e0b; border-radius:10px; padding:10px">
      ...
      <form id="list-new-form" style="display:none; gap:8px; align-items:end">
        <div>
          <label>Nombre<br><input id="list-new-name" required placeholder="Ej. Polinizadores" /></label>
        </div>
        <div>
          <label>Tipo<br>
            <select id="list-new-type">
              <option value="insectos" selected>insectos</option>
