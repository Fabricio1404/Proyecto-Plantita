...
async function loadTaxon() {
    ...
    try {
        ...
        const renderUserList=(s,u,l)=>{const e=$(s); if(!e)return; e.innerHTML=(u||[]).map(u=>{const n=u.user?.name||u.user?.login||"Usuario"; const c=u.observation_count||u.count||0; const i=u.user?.icon_url||'./assets/img/default-avatar.png'; return `<div class="card" style="flex-direction:row;align-items:center;padding:8px;gap:10px;"><img src="${i}" alt="${n}" style="width:40px;height:40px;border-radius:50%;"><div class="species-info" style="padding:0;"><p class="common" style="margin:0;font-weight:600;color:var(--text);text-shadow:none;">${escapeHTML(n)}</p><p class="sci" style="margin:0;color:var(--muted);text-shadow:none;">${c.toLocaleString("es-AR")} ${l}</p></div></div>`;}).join("");}; renderUserList("#top-observers",R.observers,"obs."); renderUserList("#top-identifiers",R.identifiers,"id.");
        const similarGrid=$("#similar"); if(similarGrid){similarGrid.innerHTML=(R.similar||[]).map(x=>{const i=x.default_photo?.medium_url||x.default_photo?.url||""; const c=x.preferred_common_name||"—"; const s=x.name||""; return `<article class="card species-card" data-id="${x.id}" tabindex="0" role="button"><img src="${i}" alt="${c}" loading="lazy" data-skeleton><div class="species-info"><h4>${escapeHTML(c)}</h4><p>${escapeHTML(s)}</p></div></article>`;}).join(""); similarGrid.addEventListener("click",(e)=>{const c=e.target.closest(".species-card"); if(!c)return; const id=c.dataset.id; if(id){location.href=`detail.html?id=${id}&from=${cameFrom}`;}}); }
