...
const buscarEspecies = async (taxonName, query = '') => {
  try {
    ...
    if (taxonName) {
        params.set('iconic_taxa', taxonName); 
    }
    ...
    const response = await fetch(url, {
        headers: { 'User-Agent': 'PlataformaEcologica/1.0' }
    });
    ...
    return data.results.map((obs) => ({
      id: obs.id,
      nombreComun:
        (obs.taxon && (obs.taxon.preferred_common_name || obs.taxon.english_common_name)) ||
        'Sin nombre común',
      nombreCientifico: (obs.taxon && obs.taxon.name) || '—',
      imagenes: (obs.photos || []).map((p) => ({
        url: p.url ? p.url.replace('square', 'medium') : null
      })),
      ubicacion: obs.location ? {
        lat: obs.latitude,
        lng: obs.longitude,
        display: obs.place_guess
      } : null,
      fechaObservacion: obs.observed_on_details?.date,
      url: obs.uri || null
    }));
  } catch (error) {
    console.error('Error CRÍTICO en buscarEspecies:', error.message);
    return [];
  }
};
