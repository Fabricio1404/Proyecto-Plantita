/**
 * Controladores relacionados con 'Clase' (crear, unirse, materiales y tareas).
 * Conserva lógica de permisos (profesor/alumno) y manejo de archivos.
 */
const fs = require('fs');
...
const crearClase = async (req, res) => {
    ...
    if (!nombre || nombre.trim().length === 0) return res.status(400).json({ ok: false, msg: 'El nombre de la clase es obligatorio.' });
    ...
    try {
        ...
        const nuevaClase = new Clase({ nombre, codigoAcceso, profesor: profesorId, alumnos: [profesorId] });
        await nuevaClase.save();

        res.status(201).json({ ok: true, msg: 'Clase creada con éxito.', clase: nuevaClase });
    } catch (error) {
        console.error(error);
...
const unirseAClase = async (req, res) => {
    ...
    if (!codigoAcceso) return res.status(400).json({ ok: false, msg: 'El código de acceso es obligatorio.' });

    try {
        const clase = await Clase.findOne({ codigoAcceso: codigoAcceso.toUpperCase() });
        if (!clase) return res.status(404).json({ ok: false, msg: 'Código de clase inválido.' });
        if (clase.alumnos.includes(alumnoId)) return res.status(400).json({ ok: false, msg: 'Ya eres miembro de esta clase.' });

        clase.alumnos.push(alumnoId);
        await clase.save();
        res.status(200).json({ ok: true, msg: 'Te has unido a la clase con éxito.', claseId: clase._id });
    } catch (error) {
        console.error(error);
...
const obtenerMisClases = async (req, res) => {
    ...
    try {
        const clases = await Clase.find({ alumnos: usuarioId }).populate('profesor', 'nombre apellido');
        res.status(200).json({ ok: true, total: clases.length, clases });
    } catch (error) {
        console.error(error);
...
const obtenerClasePorId = async (req, res) => {
    ...
    const usuarioId = req.uid;
    try {
        const clase = await Clase.findById(id).populate('profesor', 'nombre apellido email').populate('alumnos', 'nombre apellido');
        ...
        if (!esMiembro) return res.status(403).json({ ok: false, msg: 'No eres miembro de esta clase.' });
        res.status(200).json({ ok: true, clase });
    } catch (error) {
        console.error(error);
        if (error.kind === 'ObjectId') return res.status(404).json({ ok: false, msg: 'Clase no encontrada (ID inválido).' });
...
const agregarMaterial = async (req, res) => {
    ...
    if (!req.file) {
        if (req.multerError) return res.status(400).json({ ok: false, msg: req.multerError });
    ...
    try {
        ...
        if (!clase) return res.status(404).json({ ok: false, msg: 'Clase no encontrada.' });
        if (clase.profesor.toString() !== profesorId) return res.status(403).json({ ok: false, msg: 'No tienes permiso para añadir material a esta clase.' });

        const urlArchivo = req.file.path.replace(/\\/g, '/');
        const nuevoMaterial = { titulo, descripcion, urlArchivo, fechaPublicacion: new Date() };

        clase.materiales.push(nuevoMaterial);
        await clase.save();
        res.status(201).json({ ok: true, msg: 'Material añadido con éxito.', clase });
    } catch (error) {
        console.error(error);
...
const agregarTarea = async (req, res) => {
    ...
    if (!titulo) return res.status(400).json({ ok: false, msg: 'El título de la tarea es obligatorio.' });
    if (req.multerError) return res.status(400).json({ ok: false, msg: req.multerError });

    try {
        const clase = await Clase.findById(claseId);
        if (!clase) return res.status(404).json({ ok: false, msg: 'Clase no encontrada.' });
        if (clase.profesor.toString() !== profesorId) return res.status(403).json({ ok: false, msg: 'No tienes permiso para añadir tareas a esta clase.' });
        ...
        res.status(201).json({ ok: true, msg: 'Tarea añadida con éxito.', tarea: nuevaTarea });
    } catch (error) {
        console.error(error);
...
const obtenerTareasPorClase = async (req, res) => {
    const { id: claseId } = req.params;
    try {
        const tareas = await Tarea.find({ clase: claseId }).sort({ fechaVencimiento: 1 });
...
const borrarMaterial = async (req, res) => {
    ...
    try {
        const clase = await Clase.findById(claseId);
        if (!clase) return res.status(404).json({ ok: false, msg: 'Clase no encontrada.' });
        if (clase.profesor.toString() !== profesorId) return res.status(403).json({ ok: false, msg: 'No tienes permiso para borrar este material.' });
...
const editarMaterial = async (req, res) => {
    ...
    try {
        const clase = await Clase.findById(claseId);
        if (!clase) return res.status(404).json({ ok: false, msg: 'Clase no encontrada.' });
        if (clase.profesor.toString() !== profesorId) return res.status(403).json({ ok: false, msg: 'No tienes permiso para editar este material.' });
        ...
        await clase.save();
        res.json({ ok: true, msg: 'Material actualizado.', clase });
    } catch (error) {
        console.error(error);
